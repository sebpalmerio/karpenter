...
func (d *Drift) ComputeCommands(ctx context.Context, disruptionBudgetMapping map[string]int, candidates ...*Candidate) ([]Command, error) {
	...
	for _, candidate := range slices.Concat(emptyCandidates, nonEmptyCandidates) {
		...
		results, terminateFirst, err := SimulateSchedulingWithReservedFallback(ctx, d.kubeClient, d.cluster, d.provisioner, d.clock, d.recorder, nil, SimulationOptions{}, candidate, options.FromContext(ctx).FeatureGates.TerminateFirstDrift)
		if err != nil {
