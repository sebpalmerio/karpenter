...
func (r *Repair) commandForCandidate(
	...
	candidate, results, terminateFirst, ok, err := r.replacementForCandidate(
		ctx,
		candidate,
		options.FromContext(ctx).FeatureGates.TerminateFirstRepair,
	)
	if err != nil {
...
func (r *Repair) replacementForCandidate(
...
) (*Candidate, pscheduling.Results, bool, bool, error) {
	if candidate.OwnedByStaticNodePool() {
		...
		results, terminateFirst, ok := r.staticReplacement(current, terminateFirstEnabled)
		return current, results, terminateFirst, ok, nil
	}
	return r.dynamicReplacement(ctx, candidate, terminateFirstEnabled)
}
...
func (r *Repair) dynamicReplacement(
	...
	results, terminateFirst, err := SimulateSchedulingWithReservedFallback(
		ctx, r.kubeClient, r.cluster, r.provisioner, r.clock, r.recorder, nil,
		SimulationOptions{IncludeBlockedCandidatePods: true},
		candidate,
		terminateFirstEnabled,
	)
	if err != nil {
	...
	if !terminateFirst && !results.AllNonPendingPodsScheduled() {
	...
	return current, results, terminateFirst, true, nil
}
...
func (r *Repair) staticReplacement(candidate *Candidate, terminateFirstEnabled bool) (pscheduling.Results, bool, bool) {
	...
	if int64(runningNodes+pendingDisruption) > lo.FromPtr(candidate.NodePool.Spec.Replicas) {
		...
		return pscheduling.Results{}, false, false
	}
	if int64(runningNodes+pendingDisruption) >= nodeLimit {
		refillable := candidate.NodePool.StatusConditions().Root().IsTrue() && candidate.NodePool.DeletionTimestamp.IsZero()
		...
		return pscheduling.Results{}, true, true
	}
